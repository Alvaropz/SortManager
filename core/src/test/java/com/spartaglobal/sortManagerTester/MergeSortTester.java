...
public class MergeSortTester {
    ...
    @Test
    @DisplayName("This checks if an array is sorted via the Merge Sort method passing and array and N")
    public void testMergeSortWithN(){
        int[] inputArray = new int[40];
        for (int i = 0; i < inputArray.length; i++) {
            inputArray[i] = (int)Math.floor((Math.random()*100)+1);
        }
        ...
        int[] arraySorted = mergeSort.sort(inputArray, 40);

        String expected = Arrays.toString(arraySorted);
        String actual = Arrays.toString(inputArrayCopy);
        Assertions.assertEquals(expected, actual);
